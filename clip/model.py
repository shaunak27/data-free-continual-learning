...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self, x: torch.Tensor,prompt=None,vflag=False):
        self.attn_mask = self.attn_mask.to(dtype=x.dtype, device=x.device) if self.attn_mask is not None else None
        if vflag:
            pass #print(x.shape)
        return self.attn(x, x, x, need_weights=False, attn_mask=self.attn_mask,prompt=prompt,vflag=vflag)[0]

    def forward(self, x: torch.Tensor,prompt=None,vflag=False):
        x = x + self.attention(self.ln_1(x),prompt=prompt,vflag=vflag)
