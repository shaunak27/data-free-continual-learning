...
DATE=fedavg_v5.0_INR_iid_cutoff_cutratio_0.1_percent_0.6_seed_0
...
python -u run.py --config $CONFIG_CLIP_P --gpuid $GPUID --repeat $REPEAT --memory $MEMORY --overwrite $OVERWRITE --debug_mode $DEBUG \
    ...
    --prompt_param 100 20 1 -1 --kl --imbalance 1 --percent 0.6 --n_clients 5 --n_rounds 10 --cutoff --cutoff_ratio 0.1 \
    --wandb_name $DATE \
