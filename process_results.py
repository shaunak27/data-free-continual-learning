...
for same_plot in [True, False]:
    ...
    for metric_prefix in ['acc','mem','time','plastic','til','cka']:
        ...
        csv_rows = None
        metrics = ['Accuracy', 'Task FGT', 'Class FGT']
            ...
            if metric == 'Accuracy' or acc_flag:
                ...
                for name, results in results_all.items():
                    ...
                    if ins['split_size'] > 0:
                        ...
                        x = 100 + ins['split_size'] * (np.arange(num_x))
                        x = x[::-1]
                    if len(y) == 1: 
                        ...
                        offline.append(True)
                        if acc_flag: 
                            y_plot_pt.append(y_long)
                            y_plot_pt_b.append(y_long)
                    else:
                ...
                if len(final_acc) > 1:
                    ...
                    if acc_flag:
                        for j in range(len(name_plot)):
                            ...
                            results_row.extend(['{acc:.1f}'.format(acc=-1 * bwt[0])])
