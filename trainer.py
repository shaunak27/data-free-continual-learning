...
class Trainer:
    ...
    def __init__(self, args, seed, metric_keys, save_keys):
        ...
        self.generator = torch.nn.DataParallel(self.generator, device_ids=[0,1,2,3], output_device=0)
    ...
    def train_generator(self,round=0,task=0):
        ...
        #set server and clients to eval mode
        self.server.model.eval()
        for learner in self.learners:
            learner.model.eval()
        for i in pbar:
            #ENSURE RANDOMNESS !!!
            labels = torch.from_numpy(np.random.choice(self.num_classes, batch_size, p=np.array(list(self.label_counts_server.values())) / np.array(list(self.label_counts_server.values())).sum())).cuda()
            eps = torch.randn(labels.shape[0], 32).cuda()
            z = self.generator(labels,eps)
            xent_loss = 0
            kl_loss = 0
            div_loss = 0
            ...
            if self.prev_server is not None:
                logits_last = self.prev_server.model.forward(x=None,z=z)[:,tasks_till_now]
                #current_task = [j for sub in self.server.tasks_real[self.current_t_index:self.current_t_index+1] for j in sub]
                #logits_last[current_task] = -float('inf')
                #print(logits_last)
                xent_loss += ((torch.tensor(list(self.label_counts_server_last_task.values())) / torch.tensor(list(self.label_counts_server_last_task.values())).sum()).cuda()[labels] * cross_entropy(logits_last, labels)).sum()
                kl_loss += ((torch.tensor(list(self.label_counts_server_last_task.values())) / torch.tensor(list(self.label_counts_server_last_task.values())).sum()).cuda()[labels][:,None] * kl_div(torch.log_softmax(self.server.model.forward(x=None,z=z)[:,tasks_till_now], dim=1),torch.softmax(logits_last, dim=1))).sum()
            
            total_loss = 3*xent_loss - kl_loss + div_loss 
    ...
    def knowledge_distillation(self):
        ...
        optimizer2 = torch.optim.Adam(self.server.model.module.last.parameters(), lr=1e-4,betas=(0.9, 0.999),
            eps=1e-08, weight_decay=1e-2, amsgrad=False)
        #set clients and generator to eval mode
        self.generator.eval()
        for learner in self.learners:
            learner.model.eval()
        ...
        for it in pb1:
            ...
            with torch.no_grad():
                ...
                z = self.generator(labels,eps)
            for j in range(self.n_clients):
                for l in self.server.model.module.prompt.e_layers:
                    mse_loss += ((torch.tensor(list(self.label_counts[j].values())) / torch.tensor(list(self.label_counts[j].values())).sum()).cuda()[labels] * mse(self.server.model.module.prompt.forward(z,l,None,train=True,hepco=True), self.learners[j].model.module.prompt.forward(z,l,None,train=True,hepco=True))).sum() 
            
            if self.prev_server is not None:
                for l in self.server.model.module.prompt.e_layers:
                        mse_loss += ((torch.tensor(list(self.label_counts_server_last_task.values())) / torch.tensor(list(self.label_counts_server_last_task.values())).sum()).cuda()[labels] * mse(self.server.model.module.prompt.forward(z,l,None,train=True,hepco=True), self.prev_server.model.module.prompt.forward(z,l,None,train=True,hepco=True))).sum()
        ...
        for it in pb2:
            ...
            xent_loss = cross_entropy(self.server.model(x=None,z=z)[:,tasks_till_now], labels)
        ...
        for i in range(self.max_task): ## SHAUN : See learner config
            ...
            for r in range(self.n_rounds):
                ...
                for row in range(self.num_classes):
                    for idx in range(self.n_clients):
                        self.label_counts_server[row] += self.label_counts[idx][row]
                
                self.communicate()
                if self.hepco:
                    print('Pre distillation:',self.server_task_eval(i, all_tasks=True))
                ...
                #code to plot server accuracy using wandb
