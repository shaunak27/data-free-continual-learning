...
def matvec_product(W, x, bias, phm_rule, kronecker_prod=False) -> torch.Tensor:
    """
...
class PHMLinear(torch.nn.Module):
    def __init__(self,
        ...
        if not self.shared_phm_rule:
            if self.factorized_phm_rule:  # tend to false
        ...
        self.w_init = w_init
        self.c_init = c_init
        self.shared_W_phm = shared_W_phm  # B non-shared
    ...
    def init_W(self):
        if self.w_init == "glorot-normal":
            ...
            if self.factorized_phm:
                for i in range(self.phm_dim):
                    self.W_left.data[i].normal_(
                        mean=0, std=self.phm_init_range)
                    self.W_right.data[i].normal_(
                        mean=0, std=self.phm_init_range)
            else:
                for i in range(self.phm_dim):
    ...
    def forward(self, x, phm_rule=None):
        ...
        return matvec_product(
            ...
            x=x,
            bias=self.b,
            phm_rule=phm_rule if self.shared_phm_rule else self.phm_rule,
