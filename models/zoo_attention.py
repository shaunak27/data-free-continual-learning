...
# (b x 1 x d) * soft([1 x nbase x d]) = (b x nbase x d) -> attention = nbase x d

# (b x nbase x d) - [1 x nbase x d] = (b x nbase) -> key = nbase x d

# (b x 1 x nbase x 1) * [1 x plen x nbase x d] = (b x plen x d) -> prompt = plen x nbase x d
...
class DualPrompt(nn.Module):
    ...
    def forward(self, x_querry, l, x_block, train=False, task_id=None):
        ...
        if l in self.e_layers:
            ...
            if True:
                # a_querry = torch.mul(x_querry.expand(), nn.functional.softmax(A,dim=1)) # B x K x D
                a_querry = torch.einsum('bd,kd->bkd', x_querry, nn.functional.softmax(A,dim=1))
                n_K = nn.functional.normalize(K, dim=1)
                q = nn.functional.normalize(a_querry, dim=2).detach()
                cos_sim = torch.einsum('bkj,kj->bk', q, n_K)
            ...
            if train:
                ...
                k_idx = top_k.indices
                if l==0: print(torch.bincount(k_idx.flatten().detach(),minlength=self.e_pool_size))
