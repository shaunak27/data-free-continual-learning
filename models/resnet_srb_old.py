...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, num_blocks, stride, bd):
        ...
        stride_hold = stride
        for bi in range(bd):
            self.in_planes = int(in_planes_hold/bd)
            stride = stride_hold
    ...
    def forward_h(self, x):
        ...
        for l in range(1,self.block_division):
            ...
            out2 = torch.cat([out2, self.layer1[l](out1[:,bd_i:bd_i+int(bd/self.block_division)])], dim=1)
        ...
        bd = out2.size()[1]
        bd_i = 0
        out3 = self.layer2[0](out2[:,bd_i:bd_i+int(bd/self.block_division)])
        for l in range(1,self.block_division):
            bd_i += int(bd/self.block_division)
            out3 = torch.cat([out3, self.layer2[l](out2[:,bd_i:bd_i+int(bd/self.block_division)])], dim=1)
        ...
        bd = out3.size()[1]
        bd_i = 0
        out4 = self.layer3[0](out3[:,bd_i:bd_i+int(bd/self.block_division)])
        for l in range(1,self.block_division):
            bd_i += int(bd/self.block_division)
            out4 = torch.cat([out4, self.layer3[l](out3[:,bd_i:bd_i+int(bd/self.block_division)])], dim=1)
